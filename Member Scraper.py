...
async def scrape_users(guild: discord.Guild):
    ...
    if channels:
        for x in channels:
            if x.type in [discord.ChannelType.category, discord.ChannelType.forum]:
                continue
    ...
    for channel_id, count in sorted_channels[:5]:
        try:
            ...
            print(f"Failed to fetch channel {channel_id}: Forbidden (skipping)")
            continue
        except discord.HTTPException:
            print(f"Failed to fetch channel {channel_id}: HTTPException (skipping)")
            continue
    ...
    for channel_curr in tep:
        ...
        if channel_curr.type == discord.ChannelType.category: 
            continue
            
        
        if not channel_curr.permissions_for(member).read_messages:
            continue
        ...
        if channel_curr.type == discord.ChannelType.text:
            try:
    ...
    if welcome_channel_id:
        welcome_channel_id_int = int(welcome_channel_id)
        welcome_in_list = False
        
        
        for ch in channels_to_scrape:
            if ch.id == welcome_channel_id_int:
                welcome_in_list = True
                print(f" Welcome channel already in list: {ch.name}")
                break
        
        
        if not welcome_in_list:
            try:
                welcome_channel = await guild.fetch_channel(welcome_channel_id_int)
                if welcome_channel.type != discord.ChannelType.category:
                    channels_to_scrape.insert(0, welcome_channel)  
                    print(f" Force-added welcome channel: {welcome_channel.name}")
                else:
                    print(f"  Welcome channel is a category, skipping")
            except Exception as e:
                print(f" Could not fetch welcome channel: {e}")
    
    print(f"channels to scrape: {len(channels_to_scrape)}")
    print(f"First few channels: {[ch.name for ch in channels_to_scrape[:5]]}")
    ...
    try:
        for x in channels_to_scrape:
            ...
            if welcome_channel_id and x.id == int(welcome_channel_id):
                mes_depth = None  
                welcome_channel = True
                print(f"\n SCRAPING WELCOME CHANNEL: {x.name}")
                print("   This will extract users from Dyno bot mentions!")
